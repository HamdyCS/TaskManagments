...
namespace Application.Features.Tasks.Queries.GetMyTasks
{
    public class GetMyTasksQueryHandler(
        ...
        ILogger<GetMyTasksQueryHandler> logger) : IRequestHandler<GetMyTasksQuery, ErrorOr<PaginationResultDto<TaskDto>>>
    {
        public async Task<ErrorOr<PaginationResultDto<TaskDto>>> Handle(GetMyTasksQuery request, CancellationToken cancellationToken)
        {
            logger.LogInformation("Starting GetMyTasks for user {UserId} in project {ProjectId}", request.UserId, request.ProjectId);
            ...
            logger.LogInformation("GetMyTasks for user {UserId} in project {ProjectId} completed successfully", request.UserId, request.ProjectId);
